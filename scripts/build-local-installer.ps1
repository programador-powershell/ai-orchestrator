...
$bootstrapper = Join-Path $workspace 'apps\bootstrapper\src-tauri\target\release\ai-bot-bootstrapper.exe'
...
try {
  if (-not $FinalizeOnly) {
    ...
    Invoke-Pnpm --filter '@ai-bot/desktop' tauri build --config src-tauri/tauri.local.conf.json --bundles nsis
  ...
  if (-not $FinalizeOnly) {
    ...
    Invoke-Pnpm --filter '@ai-bot/bootstrapper' tauri build --no-bundle
  ...
  $setup = Join-Path $output 'AI-BOT-Setup-Local.exe'
  $copied = $false
