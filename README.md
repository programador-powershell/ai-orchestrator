...
| Data / ERD | drawDB | 🟡 Parcial | Desenhar tabelas, export SQL (5 dialetos), migrações up/down, **imagem fiel ao layout desenhado**, **import de dump real** (nome qualificado, comentário de bloco), persistência | Criar FK arrastando; o import ainda é regex, não um parser SQL completo |
| Agent | LangGraph/Flowise (client-side) | 🟡 Parcial | DAG real executando nós, **onda roda em paralelo respeitando o maxConcurrency**, **execução persiste em disco e "Retomar" reaproveita só o que terminou bem**, fusion, gate humano | Execução server-side (o fluxo morre se a aba fechar; retomar é manual) |
...
| Security / Sandbox | Snyk + execução isolada | 🟡 Parcial | Varredura de segredos e **painel do sandbox na aba Security** — roda em diretório efêmero, ambiente limpo (`env_clear`), PATH mínimo e `kill_on_drop` | Isolamento de SO real (Job Object/AppContainer): o processo ainda roda com os direitos do usuário |
