...
- **Equipe de agentes escalada pela complexidade** (aba Agent): no prompt vai só o objetivo, o fluxo é pré-determinado e segue a espinha spec-driven (constituição → spec → plano → tarefas → revisão → CI). Os nós surgem conforme os agentes são contratados e a barra lateral lista `modelo - papel` ao vivo; o modelo de cada papel é do admin.
...
| Security / Sandbox / computer use | Snyk + openclaw | 🟡 Parcial | Varredura de segredos e painel do sandbox — diretório efêmero, ambiente limpo, e **Job Object do Windows**: processo nasce suspenso, teto de processos/memória, restrições de UI e **a árvore inteira morre no fim, inclusive netos órfãos** | A **área de trabalho do agente** confina caminho (`..`, absoluto e symlink apontando para fora são recusados) e encerra a árvore de processos, mas **não reduz privilégio**: não é AppContainer nem contêiner — o comando roda com o token do usuário e alcança a rede. Ver `docs/adr-computer-use.md` |
...
| Governança e relatoria | Console corporativo (ChatGPT Enterprise) | 🟡 Parcial | Política por grupo do AD assinada em Ed25519 e verificada no Rust; **uso medido de verdade** — tokens de entrada/saída contados nos três caminhos de provedor (inclusive em streaming, sem quebrar o stream) e **custo por usuário, grupo, modelo e dia**, calculado em SQL como `numeric` para não sofrer erro de ponto flutuante; **blocklist de domínios** definida pelo admin (união entre grupos); **trilha de auditoria** de cada comando executado na máquina, em tabela própria; modelo por papel da equipe de agentes | As migrações 0006–0008 e as rotas novas do gateway **nunca rodaram contra um PostgreSQL real** — só há teste unitário do que é puro. Sem tela de orçamento/alerta: o relatório mostra o gasto, não o impede |
...
- **A aba Agent vira uma equipe, não um workflow.** No prompt vai **só o objetivo** (e correções, num campo à parte). O fluxo deixa de ser desenhado pela pessoa ou inventado pelo modelo: é **pré-determinado pela complexidade** do pedido e segue sempre a mesma espinha spec-driven — constituição → spec → plano → tarefas → revisão → CI. A classificação é uma heurística **declarada e visível**, não um modelo, porque o mesmo pedido precisa produzir a mesma equipe: uma escalação que muda a cada execução é exatamente o que este redesenho veio consertar. Os sinais aparecem na tela e dá para forçar outro nível.
...
- Novos módulos puros e testados: `lib/videoCompose` (composição ffmpeg), `lib/agentCrew` + `lib/agentCrewRun` (escalação e execução por ondas), `lib/symbols` (índice), `lib/fim` (completar no cursor), `lib/semantic` + `lib/memoryVectors` (busca por sentido) — **1294 testes** no desktop, 79 no gateway e 105 no Rust do cliente.
