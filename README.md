...
| Design | Canva / Pencil / clone de sites | 🟡 Parcial | Canvas editável com export SVG/PNG; **clona o LAYOUT REAL** — a página é renderizada e a geometria sai de `getBoundingClientRect`, não de estimativa de CSS; **stencils de prototipagem** (botão, campo, cartão, navbar, navegador, celular, fluxograma) e **conectores que acompanham as formas**; extração de paleta e fontes | **SPA não aparece**: o iframe roda sem script de propósito (HTML de terceiro não executa código no app), então tela montada por JS fica de fora. Captura é um retrato estático de uma largura só |
...
| Office | Word/Excel/PowerPoint | 🟡 Parcial | Lê DOCX, XLSX e PPTX de verdade (OOXML no Rust) e **EDITA o binário de DOCX/PPTX**: substituir texto reescreve o arquivo preservando formatação, estilos, numeração e relações — casa o texto **atravessando runs** (92% dos parágrafos do Word são fragmentados), cobre corpo/cabeçalho/rodapé/notas, ignora controle de alterações e códigos de campo, e grava de forma **atômica**. Edita HTML/MD/CSV/TXT | **Edição de XLSX** (célula depende de tabela compartilhada + aba nomeada); inserir/apagar estrutura (parágrafo, slide, comentário); formatação; **PDF** (nem lê nem escreve — exige biblioteca nova, sujeita a TI/SI) |
