...
- **O WebSocket do gateway estava quebrado em cinco dos sete comandos.** `#[serde(rename_all)]` num enum renomeia o NOME DA VARIANTE, não os campos dela: o servidor esperava `run_id`/`from_seq`/`approval_id` enquanto o cliente sempre enviou `runId`/`fromSeq`/`approvalId`. O efeito era o pior tipo de falha — a conexão autenticava, o `hello` chegava, e dali em diante todo `subscribe`, `events`, `decide` e `cancel` voltava `BAD_FRAME`. Sem inscrição e sem ingest, o canal inteiro era decorativo, e cada lado passava nos próprios testes.
- **O evento ao vivo era descartado em silêncio.** O fanout publicava `{runId, events}` **sem `type`**, o hub repassava verbatim e o `parseFrame` do cliente devolve `null` para frame sem `type`. Mesmo com o item acima resolvido, o tempo real ficaria morto sem erro, sem log e sem socket fechado — parecendo "não há eventos novos".
- **O cursor de sincronização podia congelar um run para sempre.** O `ack` devolve o `last_seq` do SERVIDOR, que é maior que o local quando outra máquina empurrou eventos do mesmo run; gravá-lo marcava como sincronizado um evento que esta máquina nem produziu, e o run saía da fila de pendências definitivamente. O cursor passou a ser preso ao `last_seq` local.
- **Criar sessão apagava o título.** `ON CONFLICT DO UPDATE SET title=excluded.title` com título ausente gravava vazio — a segunda gravação da mesma sessão apagava o nome dela.
- **O Dockerfile gerado nunca instalava dependência.** O painel montava o plano sem `installCommand`, porque quem sabe instalar (`npm ci`, `pip install -r`) é o detector de LINGUAGEM e não o de framework. A imagem rodava `next build` sem `node_modules` e quebrava no build, com o erro parecendo do projeto. Os dois detectores agora se cruzam num plano só.
- **O emulador não sabia dizer por que a sessão morreu.** Chave recusada no SSH, shell inexistente, processo morto pelo sistema: os três davam a mesma tela vazia. O estado do terminal passou a ser o redutor puro que já existia e nunca fora ligado — com status, código de saída, tráfego e log com horário num painel de diagnóstico, e a regra que o `useState` não tinha: status terminal não volta atrás.
...
- **Revisão adversarial de 140 agentes** em oito dimensões (terminal, desempenho de render, desempenho de dados, port do openship, órfãos, React Flow, rename/build e segurança), cada achado submetido a três céticos independentes: 44 brutos, 32 sobreviveram, e são eles que estão corrigidos acima. A suíte cresceu de 1.692 testes — os novos travam justamente o que a revisão encontrou: a fila da inscrição do PTY, a poda por cota, a política de motor, o Dockerfile de site estático e o cursor em texto com emoji.
- **Teste de contrato CRUZADO entre gateway e cliente.** A divergência de campo acima não aparecia em compilação nem em teste de um lado só, porque cada lado falava consigo mesmo. Agora o teste do Rust serializa os nove frames de saída numa fixture versionada, e o teste do cliente lê **o mesmo arquivo** e exige que o `parseFrame` reconheça todos. Quem mexer num dos lados quebra o outro na hora. Há também o caminho inverso: um teste que recusa `snake_case`, para a correção não ser revertida sem alguém perceber.
- **Suíte em 1.771 testes de TypeScript e 177 de Rust** (eram 1.751/172).
