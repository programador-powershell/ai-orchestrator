...
const PRODUCT: &str = "Multiplike-AI";
...
async fn signed_manifest() -> Result<ReleaseManifest, String> {
    let url = option_env!("RELEASE_MANIFEST_URL").ok_or_else(|| {
        "Este instalador é um build de desenvolvimento e não possui um canal de download. Baixe o Multiplike-AI Setup em uma Release publicada.".to_string()
    })?;
    if url.contains("__") {
        return Err(
            "Este instalador foi gerado sem configurar o canal de distribuição. Baixe novamente o Multiplike-AI Setup em uma Release publicada."
                .into(),
        );
    }
    https_only(url)?;
...
async fn run_install(app: AppHandle, state: InstallState) -> Result<(), String> {
    ...
    fs::create_dir_all(&cache)
        .await
        .map_err(|e| e.to_string())?;
    let installer = cache.join(format!("Multiplike-AI-{}-setup.exe", manifest.version));
    ...
    emit(
        ...
        0,
        Some(0),
        "Abrindo o Multiplike-AI…",
    );
    let executable = destination.join("multiplike-ai-desktop.exe");
