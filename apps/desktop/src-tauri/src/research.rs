...
pub async fn research_fetch(url: String) -> Result<FetchedPage, String> {
    ...
    let (_, body) = fetch_guarded(parsed, "Multiplike-AI Research/1.0", Duration::from_secs(15)).await?;
...
pub async fn page_fetch(url: String) -> Result<FetchedPage, String> {
    ...
    let (url_final, body) =
        fetch_guarded(parsed, "Multiplike-AI Design/1.0", Duration::from_secs(20)).await?;
