...
mod tests {
    ...
    fn troca_dentro_de_um_unico_run() {
        ...
        let (out, n) = replace_in_xml(&xml, "mundo", "Multiplike", &DOCX);
        assert_eq!(n, 1);
    ...
    fn troca_agulha_que_atravessa_dois_runs() {
        ...
        let (out, n) = replace_in_xml(&xml, "Multiplike", "ACME", &DOCX);
    ...
    fn contagem_de_tags_nao_muda() {
        ...
        let (out, _) = replace_in_xml(&xml, "Multiplike", "ACME", &DOCX);
...
mod roundtrip {
    ...
    fn edita_docx_real_preservando_o_pacote() {
        ...
        assert!(!doc.contains("Multiplike"), "texto antigo ficou: {doc}");
    ...
    fn nao_deixa_arquivo_temporario_para_tras() {
        ...
        office_replace_text(raiz, "limpo.docx".into(), "Multiplike".into(), "ACME".into())
            .expect("edição");
