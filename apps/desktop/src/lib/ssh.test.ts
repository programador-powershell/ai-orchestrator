...
describe("resolveRoute", () => {
  ...
  it("VPS com um servidor habilitado roteia por SSH", () => {
    ...
    if (rota.kind === "ssh") expect(rota.server.host).toBe("vps.aibot.local");
  });
...
describe("routeLabel", () => {
  it("mostra o destino real na barra de status", () => {
    expect(routeLabel(resolveRoute("vps", [servidor()]))).toBe("deploy@vps.aibot.local");
