...
const STORAGE_KEY = "aio.memory.vectors.v1";
...
export function loadCache(storage: Storage): VectorCache {
  try {
    const raw = storage.getItem(STORAGE_KEY);
    if (!raw) return {};
    const parsed = JSON.parse(raw) as unknown;
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return {};
    const saida: VectorCache = {};
...
export function saveCache(storage: Storage, cache: VectorCache): void {
  ...
  try {
    storage.setItem(STORAGE_KEY, JSON.stringify(Object.fromEntries(podado)));
  } catch {
...
export function missingVectors(items: MemoryItem[], cache: VectorCache): MemoryItem[] {
  return items.filter((item) => !cache[textHash(memoryText(item))]);
}
...
export async function vectorScores(options: VectorRunOptions): Promise<Map<string, number> | null> {
  const { items, query, storage, embed } = options;
  if (!items.length || !query.trim()) return null;

  const cache = loadCache(storage);
  const faltando = missingVectors(items, cache).slice(0, options.maxNew ?? 24);

  // Uma chamada só: a consulta primeiro, depois as memórias sem vetor.
  const entradas = [query, ...faltando.map((item) => memoryText(item))];
  const vetores = await embed(entradas);
  if (!vetores || !vetores.length) return null;

  const daConsulta = vetores[0];
  faltando.forEach((item, index) => {
    const vetor = vetores[index + 1];
    if (vetor) cache[textHash(memoryText(item))] = vetor;
  });
  if (faltando.length) saveCache(storage, cache);

  const notas = new Map<string, number>();
  for (const item of items) {
    const vetor = cache[textHash(memoryText(item))];
