...
mod tests {
    ...
    fn troca_dentro_de_um_unico_no() {
        ...
        let (out, trocas) = replace_in_xml(&xml, "mundo", "AI-BOT", &DOCX_DIALECT);
        assert_eq!(trocas, 1);
    ...
    fn troca_agulha_que_atravessa_dois_nos() {
        let xml = paragrafo("<w:r><w:t>Ola Docu</w:t></w:r><w:r><w:t>mento hoje</w:t></w:r>");
        let (out, trocas) = replace_in_xml(&xml, "Documento", "ACME", &DOCX_DIALECT);
    ...
    fn contagem_de_tags_nao_muda() {
        let xml = paragrafo(
            "<w:r><w:rPr><w:b/></w:rPr><w:t>Ola Docu</w:t></w:r><w:r><w:t>mento</w:t></w:r>",
        );
        let (out, _) = replace_in_xml(&xml, "Documento", "ACME", &DOCX_DIALECT);
    ...
    fn corpus_document_xml(paragrafos: usize) -> String {
        ...
        for indice in 0..paragrafos {
            ...
            if indice % 4 == 0 {
                ...
                out.push_str(r#"<w:r><w:t>a Docu</w:t></w:r><w:r><w:t>mento </w:t></w:r>"#);
            } else {
    ...
    fn bench_replace_in_xml_documento_grande() {
        ...
        let (tempo, (saida, trocas)) =
            crate::bench::median(|| replace_in_xml(&xml, "AI-BOT", "ACME S.A.", &DOCX_DIALECT));
