...
describe("extractTokens — cores", () => {
  ...
  it("normaliza rgb/rgba/hsl/hsla removendo espaços", () => {
    const { colors } = extractTokens(
      "color: rgb(10, 20, 30); border-color: RGB(10,20,30); background: hsla(200, 50%, 40%, .5)"
    );
...
describe("extractTokens — fonte real colada", () => {
  ...
  /** O caso que o painel de tokens do AI-BOT recebe hoje: CSS de :root puro. */
  it("processa um bloco :root exportado (variáveis CSS)", () => {
    const source = `:root {
      --primary: #2563eb;
      --ink: #111827;
      --radius: 8px;
    ...
    .btn { background: var(--primary); color: #2563eb; }`;
    const tokens = extractTokens(source);
    // var(--primary) não vira cor; o hex da variável e o do uso somam juntos.
    expect(tokens.colors[0]).toEqual({ value: "#2563eb", count: 2 });
    expect(tokens.fonts).toEqual(["Inter", "sans-serif"]);
    expect(tokens.spacing.map((s) => s.value)).toContain("8px");
  });
