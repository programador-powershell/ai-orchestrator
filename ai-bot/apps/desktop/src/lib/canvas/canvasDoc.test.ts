...
describe("moveNode / resizeNode", () => {
  it("moveNode muda só a posição", () => {
    const doc = addNode(createDoc(), rect("rect-1", 10, 20));
    const node = moveNode(doc, "rect-1", 200, 300).nodes.find((n) => n.id === "rect-1");
    expect(node).toMatchObject({ x: 200, y: 300, w: 100, h: 50 });
  });

  it("resizeNode muda só o tamanho e respeita o mínimo", () => {
    const doc = addNode(createDoc(), rect("rect-1", 10, 20));
    const node = resizeNode(doc, "rect-1", 2, 400).nodes.find((n) => n.id === "rect-1");
    expect(node).toMatchObject({ x: 10, y: 20, w: MIN_NODE_SIZE, h: 400 });
  });

  it("id desconhecido devolve o mesmo doc — arrasto de nada não gera undo", () => {
    const doc = createDoc();
    expect(moveNode(doc, "nope", 1, 1)).toBe(doc);
    expect(resizeNode(doc, "nope", 10, 10)).toBe(doc);
  });
});
...
describe("serializeDoc / parseDoc", () => {
  ...
  it("normaliza nós fora do padrão ao restaurar (tamanho mínimo)", () => {
    const restored = parseDoc('{"name":"x","nodes":[{"id":"a","type":"rect","x":0.6,"y":0,"w":1,"h":1,"fill":"#000"}]}');
    expect(restored?.nodes[0]).toMatchObject({ x: 1, w: MIN_NODE_SIZE, h: MIN_NODE_SIZE });
  });
});
