 ...
 * Portado do AI-Orchestrator (apps/desktop/src/lib/stencils.ts).
 *
 * O canvas já tem as primitivas (retângulo, elipse, texto). O que faltava
...
/**
 * Paleta neutra de wireframe — cinza legível, não decisão de marca. São hex
 * LITERAIS de propósito: custom property de tema aqui quebraria o export SVG
 * (que roda fora do DOM) e cairia na armadilha conhecida do projeto de animar
 * cor que troca por tema.
 */
const INK = "#111827";
const MUTED = "#6b7280";
const LINE = "#d1d5db";
...
/**
 * Posição de inserção em cascata (portada do insertStencil do orquestrador):
 * dois cliques seguidos precisam produzir duas peças VISÍVEIS, não uma pilha
 * exata no mesmo ponto que parece inserção falhada. O módulo 6 recicla a
 * diagonal antes de fugir da área visível.
 */
export function stencilInsertPosition(existingCount: number): { x: number; y: number } {
  const step = existingCount % 6;
  return { x: 60 + step * 24, y: 60 + step * 24 };
}
...
export function connectorsToSvg(geometry: readonly ConnectorGeometry[]): string {
  ...
  const marker =
    '<defs><marker id="aib-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#6b7280"/></marker></defs>';
  const linhas = geometry
    .map(
      (item) =>
        `<line x1="${item.x1}" y1="${item.y1}" x2="${item.x2}" y2="${item.y2}" stroke="#6b7280" stroke-width="1.5" marker-end="url(#aib-arrow)"/>` +
        (item.label
