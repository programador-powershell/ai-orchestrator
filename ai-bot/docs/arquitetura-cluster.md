...
| Staging + fence/epoch | :white_check_mark: Feito (universal, local) | A cerca roda DE VERDADE na aceitação do resultado (`runWorker` → `Promote`), com época PERSISTIDA da frota (lease com TTL de 3min; vencido = época anda, nunca volta — sobrevive a reinício). E o STAGING é UNIVERSAL (`internal/workspace/staging.go`): TODO turno de MODELO com raiz definida — provisionada OU apontada pela pessoa — trabalha numa CÓPIA em `<dataDir>/staging/<plano>/`; a cópia de ida EXCLUI os reproduzíveis (mesma lista do espelho — node_modules não estoura o teto à toa) e só o sucesso promove (espelho: novo/alterado copiado, sumido apagado); falha/interrupção/recusa/narração descartam. Teto 128 MiB / 4096 arquivos degrada para inplace com AVISO ALTO — e degradado volta à aprovação por comando. Na jaula, o gesto de arquivo/container NÃO pergunta (chip "no sandbox", `supervisor/jaula.go` — segredo e rede sensível NUNCA relaxam; os gestos de HOST office.*/video.* só relaxam quando a instalação declara `Deps.HostHonraRoot`, e o desktop ainda NÃO lê o `root` injetado — até lá eles seguem pedindo por comando, senão o efeito cairia no projeto real sem cartão) e a aprovação humana é UMA, no cartão de ENTREGA (diff criados/alterados/apagados; recusa descarta e fecha honesto; equipe cai no mesmo cartão do turno). UI e equipe (worktree é o isolamento dela) seguem inplace, de propósito. O staging REMOTO (Puter) continua na linha acima. |
