...
| Staging + fence/epoch | :white_check_mark: Feito (v1 local) | A cerca roda DE VERDADE na aceitação do resultado (`runWorker` → `Promote`), com época PERSISTIDA da frota (lease com TTL de 3min; vencido = época anda, nunca volta — sobrevive a reinício). E o STAGING v1 existe (`internal/workspace/staging.go`): turno de MODELO sobre raiz em `<dataDir>/projects/` trabalha numa CÓPIA em `<dataDir>/staging/<plano>/` e só o sucesso promove (espelho: novo/alterado copiado, sumido apagado); falha/interrupção/recusa/narração descartam. Teto 128 MiB / 4096 arquivos degrada para inplace com aviso. UI, equipe (worktree é o isolamento dela) e raiz apontada pela pessoa seguem inplace, de propósito. O staging REMOTO (Puter) continua na linha acima. |
