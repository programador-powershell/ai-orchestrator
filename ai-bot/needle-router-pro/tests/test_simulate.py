...
class RouterProTest(unittest.TestCase):
 ...
 def test_complete_vercel_application_routes_to_app_development(self):
  row={"text":"crie uma aplicação para o vercel, não apenas um site uma aplicação completa","context":{"environment":"vercel"}}
  specialty, capability, confidence, clarification=sim.classify(row,(1,1.8,2.2,2.5,.2))
  self.assertEqual((specialty,capability,clarification),("code","app-development",False))
  self.assertGreaterEqual(confidence,.78)
